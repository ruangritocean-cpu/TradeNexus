...
def render_portfolio_ui(db_path: str = None, watchlist_path: str = None):
    ...
    with col_m1:
        st.metric("Total Equity", f"${settings.account_equity:,.2f}")
    with col_m2:
        st.metric("Realized Daily Risk", f"${exposure.realized_daily_risk:,.2f}")
    with col_m3:
        st.metric("Total Open Risk", f"${exposure.total_open_risk:,.2f} ({exposure.total_open_risk_pct:.2f}%)")
    with col_m4:
        st.metric("Potential Setup Risk", f"${exposure.potential_setup_risk:,.2f} ({exposure.potential_setup_risk_pct:.2f}%)")
        
    # Block reasons or warnings
    if limits.reasons:
        for r in limits.reasons:
            st.error(f"🚨 **Block Event:** {r}")
    if limits.warnings:
    ...
    # ----------------- 2. EXPOSURE DETAILS -----------------
    st.markdown("---")
    st.subheader("📈 Exposure Breakdowns")
    
    col_exp1, col_exp2 = st.columns(2)
    with col_exp1:
        st.markdown("##### Risk By Symbol")
        if not exposure.risk_by_symbol:
            st.info("No open or pending risk exposure.")
        else:
            df_sym_risk = pd.DataFrame([{"Symbol": k, "Risk Dollar": f"${v:,.2f}"} for k, v in exposure.risk_by_symbol.items()])
            st.dataframe(df_sym_risk, use_container_width=True)
    ...
    with col_exp2:
        ...
        if not exposure.risk_by_asset_class:
            ...
            df_class_risk = pd.DataFrame([{"Asset Class": k, "Risk Dollar": f"${v:,.2f}"} for k, v in exposure.risk_by_asset_class.items()])
            st.dataframe(df_class_risk, use_container_width=True)
    ...
    with st.form("calc_form"):
        ...
        with col_calc1:
            c_entry = st.number_input("Entry Price", min_value=0.01, value=100.0, step=1.0)
        with col_calc2:
            c_sl = st.number_input("Stop Loss Price", min_value=0.01, value=95.0, step=1.0)
        with col_calc3:
    ...
    if not correlation.correlation_matrix:
        ...
        st.dataframe(df_corr.style.background_gradient(cmap="coolwarm", axis=None), use_container_width=True)
